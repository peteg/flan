...
static void
usage_error(char *progname, char *fmt, ...)
{
  va_list argp;
  ...
  va_end(argp);
  fprintf(stdout,
            "Usage: %s [-c] [-e] [-f] [-l] [-p] [-t] [sourcefile.c]\n\n\
  ...
  -g   Interpret \"main\" as a FLANGE picture. (default: don't)\n\
  -l   Evaluate the program lazily. (default: evaluate it strictly)\n\
  -p   Print the parsed version of the program. (default: don't)\n\
  -r   Print the top-level of an evaluated data constructor value as a tree. (default: don't)\n\
  -t   Don't type check the program. (default: do). THIS IS UNSAFE.\n\
  -w   Print the scanned lexemes. (default: don't)\n\
  \n",
          progname);
